    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        const newMsg = {
          ...
          isModeration: payload.new.is_moderation || false,
                    ...
                    if (msg.isModeration) {
                      return (
                        <div key={msg.id} className="flex justify-center">
                          <div className="max-w-[85%] bg-red-50 border border-red-200 rounded-xl px-4 py-2.5">
                            <p className="text-[11px] font-bold text-red-600 flex items-center gap-1 mb-0.5"><Shield size={12} /> Modération · MonRépétiteur</p>
                            <p className="text-sm text-red-900 leading-relaxed whitespace-pre-wrap break-words">{msg.content}</p>
                            <p className="text-[10px] text-red-400 mt-1">{formatTime(msg.timestamp)}</p>
                          </div>
                        </div>
                      )
                    }
